    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          title: const Text('Şifre Girin', style: TextStyle(color: Colors.white)),
          content: TextField(
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Otomasyon başlatıldı!'), backgroundColor: Colors.green),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Hata: ${response.statusCode} - ${response.reasonPhrase}'), backgroundColor: Colors.red),
          );
        }
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Bağlantı hatası: Sunucu çalışmıyor olabilir. Hata: $e'), backgroundColor: Colors.red),
        );
      } finally {
        setState(() {
            ...
            _otomasyonCalisiyor
                ? const Column(
                    children: [
                      ...
                      SizedBox(height: 20),
                      Text('Otomasyon başlatılıyor...', style: TextStyle(fontSize: 16)),
                    ],
                  )
                : ElevatedButton(
                    onPressed: () {
